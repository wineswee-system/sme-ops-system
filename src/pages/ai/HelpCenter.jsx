...
const FAQ_ITEMS = [
  ...
  { q: '如何將線索轉換為正式客戶？', a: '於「CRM > 線索管理」選擇合格的線索，點選「轉換」按鈕，即可自動建立客戶及相關商機，並保留原始線索紀錄。' },
  { q: 'AI 行銷文案如何使用？', a: '在「行銷活動」編輯頁點選「AI 生成」按鈕，輸入活動目標與目標客群，AI 會產出多版 Email/LINE/SMS 文案供您選擇或再編輯。' },
  { q: '退貨（RMA）流程怎麼走？', a: '於「WMS > 退貨管理」建立退貨單 → 收貨 → 品質檢驗（通過則重新入庫、失敗則報廢），系統會自動更新庫存與產生相關事件。' },
  { q: '組合商品（Kitting）如何建立？', a: '至「WMS > 組合商品」新增 Kit，設定組成元件與數量，系統會依元件庫存即時計算可組裝數量。' },
  { q: '客戶健康分數怎麼計算？', a: '基於 RFM（最近購買/頻率/金額）加上互動度與客服紀錄綜合評分（0-100 分）。低於門檻會自動納入流失風險清單。' },
  { q: '需求預測有哪些演算法？', a: '系統提供三種：簡單移動平均（SMA）、加權移動平均（WMA）、季節性分解（適用於有週/月循環的商品），並自動計算安全庫存與再訂購點。' },
  { q: '自動補貨怎麼觸發？', a: '當庫存低於再訂購點時，系統自動發出 wms.auto_reorder.triggered 事件，可設定自動產生採購單草稿或通知採購人員。' },
  { q: '活動時間軸可以記錄哪些互動？', a: '支援通話、會議、拜訪、Email、LINE、聊天、社群、任務、追蹤、備註等多種類型，並可設定計畫/進行中/完成/取消狀態。' },
]
